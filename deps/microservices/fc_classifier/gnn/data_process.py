"""
安装DeepHunt的数据处理方法,将数据处理为GCN的输入格式
"""
import copy
import json
import os
...
from microservices.fc_classifier.fcvgae.libs import D1, D2, save_pkl
import typing
import torch
...
def process_label(loader: typing.Union[D1, D2], window_size=10, process_type="train",train_rate=0.3):
    """只处理有label的数据
    process_type: one of train, test
    """
    samples = loader.load_all_samples()
    if process_type == "train":
        process_cases, _ = loader.load_train_test_cases(train_rate)
    else:
        _, process_cases = loader.load_train_test_cases(train_rate)
...
def process_meta_json(*, loader: typing.Union[D1, D2], train_data, test_data, graph_adj_list, n_nodes):
    ...
    data = {
        'chunk_lenth': window_size, 'chunk_num': len(train_data) + len(test_data),
...
def process_data(dataset="D1", fd_num=0.3):
    ...
    if dataset == "D1":
        ...
        loader = D2()
    train_case, test_case = loader.load_train_test_cases(train_rate=0.6)
    print("train test rate: ", len(train_case), len(test_case))
    train_data = process_label(loader, window_size=10, process_type="train",train_rate=0.6)
    test_data = process_label(loader, window_size=10, process_type="test",train_rate=0.6)
