...
# -*- coding: utf-8 -*-
"""
RandomForest分类器 - 支持ConfigurationSpace的AutoSklearn兼容分类器
...
class RandomForestClassifierAutoSklearn(AutoSklearnClassificationAlgorithm):
    """
    RandomForest分类器 - 支持ConfigurationSpace的AutoSklearn兼容分类器
    """
...
if __name__ == "__main__":
    # 测试代码
    print("开始测试RandomForest分类器...")
    
    # 创建分类器实例
    classifier = RandomForestClassifierAutoSklearn(
        dataset_name="D1",
    ...
    # 测试超参数空间
    print("\n测试超参数搜索空间...")
    cs = RandomForestClassifierAutoSklearn.get_hyperparameter_search_space()
    ...
    # 测试属性
    properties = RandomForestClassifierAutoSklearn.get_properties()
    print(f"\n分类器属性: {properties}")
    
    # 测试数据库存储
    db = KVDBJson("test_rf_results.json")
    keys = "test_rf_config"
