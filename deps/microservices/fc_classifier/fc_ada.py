...
# -*- coding: utf-8 -*-
"""
AdaBoost分类器 - 支持ConfigurationSpace的AutoSklearn兼容分类器
...
class AdaBoostClassifierAutoSklearn(AutoSklearnClassificationAlgorithm):
    """
    AdaBoost分类器 - 支持ConfigurationSpace的AutoSklearn兼容分类器
    """
...
if __name__ == "__main__":
    # 测试代码
    print("开始测试AdaBoost分类器...")

    # 创建分类器实例
    classifier = AdaBoostClassifierAutoSklearn(
        dataset_name="D1",
        n_estimators=50,
        learning_rate=1.0,
        algorithm="SAMME.R",
    ...
    # 测试超参数空间
    print("\n测试超参数搜索空间...")
    cs = AdaBoostClassifierAutoSklearn.get_hyperparameter_search_space()
    ...
    # 测试属性
    properties = AdaBoostClassifierAutoSklearn.get_properties()
    print(f"\n分类器属性: {properties}")

    # 测试数据库存储
    db = KVDBJson("test_adaboost_results.json")
    keys = "test_adaboost_config"
