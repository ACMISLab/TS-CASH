...
from pyutils.util_cuda import get_device
...
class GATClassifierAutoSklearn(AutoSklearnClassificationAlgorithm):
    """
    ...
    def get_hyperparameter_search_space(
        ...
        num_heads = UniformIntegerHyperparameter(
            "num_heads", 1, 16, default_value=8
        )
    ...
    for dataset_name in ["D1", "D2"]:
        for config in configs:
            ...
            if db.is_exist(keys):
                continue
            # 创建分类器实例
            classifier = GATClassifierAutoSklearn(
                debug=debug,
