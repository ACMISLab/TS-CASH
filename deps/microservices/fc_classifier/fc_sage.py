...
# -*- coding: utf-8 -*-
"""
GraphSAGE分类器模块 - 支持ConfigurationSpace的AutoSklearn兼容分类器
"""
import warnings
...
class GraphSAGEClassifierAutoSklearn(AutoSklearnClassificationAlgorithm):
    """
    GraphSAGE分类器 - 支持ConfigurationSpace的AutoSklearn兼容分类器
    """
...
if __name__ == '__main__':
    # 测试代码
    seed_everything(42)
    cs = GraphSAGEClassifierAutoSklearn.get_hyperparameter_search_space()
    configs = cs.sample_configuration(50)
    print(configs[0].get_dictionary())
    db = KVDBJson("graphsage_pre.json")
    train_ratio=0.1
    debug=False
    for dataset_name in ["D1", "D2"]:
        for config in configs:
            ...
            if db.is_exist(keys):
                continue
            # 创建分类器实例
            classifier = GraphSAGEClassifierAutoSklearn(
                debug=debug,
