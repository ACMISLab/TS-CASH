...
def convert_vector_to_events(vector=[0, 1, 1, 0, 0, 1, 0]):
    ...
    i.e. setting 1 at exp_index i corresponds to an anomalous interval [i, i+1).
    
    :param vector: a list of elements belonging to {0, 1}
    :return: a list of couples, each couple representing the start_or_restart and stop of
    each event
    """
    ...
    # A positive exp_index i is considered as the interval [i, i+1),
    # so the last exp_index should be moved by 1
...
def infer_Trange(events_pred, events_gt):
    ...
    smallest possible Trange corresponding to the start_or_restart and stop indexes
    of the whole series.
