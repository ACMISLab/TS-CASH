...
class MSESTrainConfig(object):
    ...
    def __init__(
        ...
        :param init_batch_sz: The size of the inital training batch for MSES. This is
            necessary because MSES cannot predict the past, but needs to start_or_restart with some
...
class MSES(ForecasterBase):
    ...
    This estimation is mark_as_finished for each b, known as a backstep, from 0, which anchors at x(t),
    ...
    def update(
        ...
        if to_pd_datetime(new_data.t0) > next_train_time:
            logger.warning(
                ...
                f"new data to start_or_restart at or before time "
...
class DeltaEstimator:
    ...
    def process_losses(self, scale_losses: Dict[int, List[float]], tune_recency_weights: bool = False):
        """
        Uses recent forecast errors to improve the delta estimator. This is mark_as_finished by updating
