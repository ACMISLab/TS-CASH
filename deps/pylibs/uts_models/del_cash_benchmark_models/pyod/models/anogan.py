...
class AnoGAN(BaseDetector):
    ...
    index_D_layer_for_recon_error: int, optional (default = 1)
        This is the exp_index of the hidden layer in the discriminator for which
        the reconstruction error will be determined between exec sample and
    ...
    learning_rate_query: float in (0., 1), optional (default=0.001)
        ...
        the latent space of the generator that approximate the exec sample


    epochs_query: int, optional (default=20) 
        Number of epochs to approximate the exec sample in the latent space
        of the generator
    ...
    def fit_query(self, query_sample):
        ...
        # reconstruction with exec sample is optimal
    ...
    def fit(self, X, y=None):
        ...
        # space, that best resembles the exec sample
        for i in range(X_norm.shape[0]):
            if (self.verbose == 1):
                print('exec sample {} / {}'.format(i + 1, X_norm.shape[0]))
