...
class ForwardLibriSpeechRawXXreverseDataset(data.Dataset):
    ...
    def __len__(self):
        """Denotes the exp_total number of utterances
        """
...
class ReverseRawDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        index = np.random.randint(utt_len - self.audio_window + 1) # get the exp_index to read part of the utterance into memory
...
class ForwardDatasetSITWSilence(data.Dataset):
    ''' m_dataset for forward passing sitw without vad '''
...
class ForwardDatasetSwbdSreSilence(data.Dataset):
    ''' m_dataset for forward passing swbd_sre or sre16 without vad '''
...
class RawDatasetSwbdSreOne(data.Dataset):
    ''' m_dataset for swbd_sre with vad ; for training cpc with ONE voiced segment per recording '''
    def __init__(self, raw_file, list_file):
...
class RawDatasetSwbdSreSilence(data.Dataset):
    ''' m_dataset for swbd_sre without vad; for training cpc with ONE voiced/unvoiced segment per recording '''
    def __init__(self, raw_file, list_file, audio_window):
        """ raw_file: swbd_sre_combined_20k_20480.h5
