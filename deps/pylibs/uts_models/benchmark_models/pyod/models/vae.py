...
class VAE(BaseDetector):
    ...
    def __init__(self, encoder_neurons=None, decoder_neurons=None,
        ...
        if self.encoder_neurons is None:
            self.encoder_neurons = [128, 64]

        if self.decoder_neurons is None:
            self.decoder_neurons = [64, 128]
    ...
    def fit(self, X, y=None):
        ...
        self.history_ = self.model_.fit(X,
                                        epochs=self.epochs,
    ...
    def fit_early_stopping(self, X, y=None):
        """Fit detector. y is optional for unsupervised methods.
    ...
    def decision_function(self, X):
        ...
        fill_score = np.nan_to_num(pred_scores)
        return pairwise_distances_no_broadcast(X, fill_score)
