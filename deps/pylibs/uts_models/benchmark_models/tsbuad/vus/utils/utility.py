...
def walk_tree(forest, node, treeIdx, obsIdx, X, featureDistrib, depth=0, alpha=1e-2):
    ...
    node: DiFF Tree node
        the current node
    treeIdx: int
        exp_index of the tree that is being walked.
    obsIdx: array
...
def argmaxn(value_list, n, order='desc'):
    """Return the exp_index of top n elements in the list
    if order is set to 'desc', otherwise return the exp_index of n smallest ones.
    Parameters
    ----------
    ...
    index_list : numpy array of shape (n,)
        The exp_index of the top n elements.
    """
    ...
    else:  # return the exp_index of n smallest elements
