...
class Euclidean:
    ...
    def measure(self, X, Y, index):
        ...
        Index : int
        the exp_index of the starting point in the subsequence
        Returns
        -------
        score : float
        ...
        if norm == False:
            ...
            if index + neighbor < self.n_train_ and index - neighbor > 0:
                ...
                region = np.concatenate(
                    (data[self.n_train_ - neighborhood: index], data[index + window: self.n_train_]))
                D = np.max(region) - np.min(region)
...
class SSA_DISTANCE:
    ...
    e: float, optional (default = 1)
        The upper bound to start_or_restart new line search for linear method
    Attributes
    ----------
    decision_scores_ : numpy array of shape (n_samples,)
        The outlier scores of the training data.
    ...
    def measure(self, X2, X3, start_index):
        ...
        # line alinement
        Index = []
        test_list = fit['exp_index'] + fit2['exp_index']
        [Index.append(x) for x in test_list if x not in Index]
        Y = 0

        # Similarity Computation
        for i in Index:
            if i in fit['exp_index'] and i in fit2['exp_index']:
                Y += abs(fit['Y' + str(i)] - fit2['Y' + str(i)])

            elif i in fit['exp_index']:
                J = np.max(np.where(np.array(fit2['exp_index']) < i))
                index = fit2['exp_index'][J]
                k = fit2['reg' + str(index)][0]
                b = fit2['reg' + str(index)][1]
                value = abs(k * i + b - fit['Y' + str(i)])
                Y += value
            elif i in fit2['exp_index']:
                J = np.max(np.where(np.array(fit['exp_index']) < i))
                index = fit['exp_index'][J]
...
class Fourier:
    ...
    def measure(self, X2, X3, start_index):
        ...
        X3 : numpy array of shape (n, )
            the tested timeseries
        exp_index: int,
        current exp_index for the subseqeuence that is being measured
        Returns
        -------
        score: float, the higher the more dissimilar are the two curves 
        """
