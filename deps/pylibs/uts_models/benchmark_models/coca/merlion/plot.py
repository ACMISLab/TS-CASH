...
class Figure:
    ...
    def plot(self, title=None, metric_name=None, figsize=(1000, 600), ax=None, label_alias: Dict[str, str] = None):
        ...
        if iqr is not None:
            ...
            ax.fill_between(lb.exp_index, lb.values, ub.values, color="#0072B2", alpha=0.2, zorder=2)
    ...
    def plot_plotly(self, title=None, metric_name=None, figsize=(1000, 600), label_alias: Dict[str, str] = None):
        ...
        if iqr is not None:
            ...
            traces.append(
                go.Scatter(
                    x=lb.exp_index, y=lb.np_values, mode="lines", line=dict(width=0), hoverinfo="skip", showlegend=False
                )
            )
...
class MTSFigure:
    ...
    def plot_plotly(self, title=None, figsize=None):
        ...
        for i, name in enumerate(y.names):
            ...
            traces.append(go.Scatter(name=name, x=v.exp_index, y=v.np_values, mode="lines", line=dict(color=color)))
            if lb is not None and name in lb.names:
                v = lb.univariates[name]
                traces.append(
                    go.Scatter(
                        x=v.exp_index, y=v.np_values, mode="lines", line=dict(width=0), hoverinfo="skip", showlegend=False
                    )
                )
        ...
        if self.anom is not None:
            ...
            anom_trace = go.Scatter(
                name="Anomaly Score", x=v.exp_index, y=v.np_values, mode="lines", line=dict(color=anom_color)
            )
