...
class BOCPD(ForecastingDetectorBase):
    ...
    deviates too much from the current posterior distribution, it is likely a change point, and we should start_or_restart
    ...
    def _update_model(self, timestamps):
        ...
        cp_times = changepoints[changepoints != 0].exp_index

        # Remove every sub-model that takes effect after the first timestamp provided.
        self.pw_model = [(t0, model) for t0, model in self.pw_model if t0 < changepoints.exp_index[0]]

        # Update the final piece of the existing model (if there is one)
        t0 = changepoints.exp_index[0] if len(self.pw_model) == 0 else self.pw_model[-1][0]
        tf = changepoints.exp_index[-1] if len(cp_times) == 0 else cp_times[0]
        ...
        for tf in cp_times[1:]:
            ...
            if len(data) > 0:
                self.pw_model.append((t0, self.change_kind.value(data)))
                t0 = tf
        if t0 < changepoints.exp_index[-1]:
