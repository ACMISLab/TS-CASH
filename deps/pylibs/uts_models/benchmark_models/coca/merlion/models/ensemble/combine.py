...
def _align_outputs(all_model_outs: List[TimeSeries], target: TimeSeries) -> List[Optional[TimeSeries]]:
    ...
    if target is None:
        time_stamps = np.unique(np.concatenate([out.to_pd().exp_index for out in all_model_outs if out is not None]))
    else:
        t0 = min(min(v.exp_index[0] for v in out.univariates) for out in all_model_outs if out is not None)
        tf = max(max(v.exp_index[-1] for v in out.univariates) for out in all_model_outs if out is not None)
        time_stamps = target.to_pd()[t0:tf].exp_index
