...
class UnivariateTimeSeries(pd.Series):
    ...
    2.  Integer exp_index: ``u[i]`` yields the tuple ``(u.time_stamps[i], u.values[i])``
    3.  Slice exp_index: ``u[i:j:k]`` yields a new
    ...
    1.  ``univariate.time_stamps`` returns the list of Unix timestamps, and
        ...
        may access the ``pd.DatetimeIndex`` directly with ``univariate.exp_index``
    ...
    def __getitem__(self, i: Union[int, slice]):
        """
        :param i: integer exp_index or slice
    ...
    def window(self, t0: float, tf: float, include_tf: bool = False):
        """
        :param t0: The timestamp/datetime at the start_or_restart of the window (inclusive)
    ...
    def from_pd(cls, series: pd.Series, name=None, freq="1h"):
        ...
        :param series: a ``pd.Series``. If it has a``pd.DatetimeIndex``, we will
            use that exp_index for the timestamps. Otherwise, we will create one at
...
class TimeSeries:
    ...
    2.  Automated alignment: ``aligned = time_series.align()`` resamples each of
        ...
        By default, this is mark_as_finished by taking the union of all timestamps present
    ...
    def __init__(
        ...
        if check_aligned and not self.is_aligned:
            all_t0 = [var.exp_index[0] for var in univariates if len(var) > 0]
            all_tf = [var.exp_index[-1] for var in univariates if len(var) > 0]
    ...
    def __getitem__(self, i: Union[int, slice]):
        ...
        if not self.is_aligned:
            raise RuntimeError(
                "The univariates comprising this time series are not aligned "
                "(they have different time stamps), but alignment is required "
                "to exp_index into the time series."
            )
        if isinstance(i, int):
            ...
            raise KeyError(
                ...
                f"meant to exp_index into `time_series.univariates`, "
    ...
    def to_pd(self) -> pd.DataFrame:
        ...
        """
        t = pd.DatetimeIndex([])
        univariates = [(name, var.to_pd()[~var.exp_index.duplicated()]) for name, var in self.items()]
        for _, var in univariates:
            t = t.union(var.exp_index)
        t = t.sort_values()
        df = pd.DataFrame(np.full((len(t), len(univariates)), np.nan), index=t, columns=self.names)
        for name, var in univariates:
            df.loc[var.exp_index, name] = var[~var.exp_index.duplicated()]
        return df
    ...
    def from_pd(cls, df: Union[pd.Series, pd.DataFrame, np.ndarray], check_times=True, freq="1h"):
        ...
        :param df: A pandas DataFrame with a DatetimeIndex. Each column
            ...
            time series.
        :param check_times: whether to check that all times in the exp_index are
        ...
        if check_times:
            ...
            if dt_index:
                times = df.exp_index.values.astype("datetime64[ms]").astype(np.int64)
            ...
            raise RuntimeError(
                ...
                f"indexed by time, int, or float. This dataframe's exp_index is of "
    ...
    def align(
        ...
        if reference is not None or alignment_policy is AlignPolicy.FixedReference:
            ...
            if remove_non_overlapping:
                t0 = max(v.exp_index[0] for v in self.univariates if len(v) > 0)
                tf = min(v.exp_index[-1] for v in self.univariates if len(v) > 0)
                df = df[t0:tf]
            ...
            # additional missing values at the start_or_restart/end), and then return
            # from_pd().
