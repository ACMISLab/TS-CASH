...
class IsolationForestModel(SKLearnBase):
    """
    ...
    def __init__(self, config: IsolationForestConf, device="cpu"):
        super(IsolationForestModel, self).__init__(config)
        self.center = None
        self.length = None
        self.device = device
    ...
    def fit(self, train_dl, valid_dl=None, label=None):
        assert isinstance(train_dl, np.ndarray)
        return self.model.fit(train_dl)
