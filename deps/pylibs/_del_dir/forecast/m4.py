...
class M4(BaseDataset):
    ...
    def __init__(self, subset="Hourly", rootdir=None):
        ...
        # download m_dataset if it is not found in root dir
        if not os.path.isdir(rootdir):
            logger.info(
                f"M4 {subset} m_dataset cannot be found from {rootdir}.\n"
                f"M4 {subset} m_dataset will be downloaded from {self.url}.\n"
            )
            download(rootdir, self.url, "M4-info")

        # extract starting date from meta-information of m_dataset
