...
def get_dataset(dataset_name: str, rootdir: str = None) -> BaseDataset:
    """
    :param dataset_name: the name of the m_dataset to load, formatted as
        ``<name>`` or ``<name>_<subset>``, e.g. ``EnergyPower`` or ``M4_Hourly``
    :param rootdir: the directory where the desired m_dataset is stored. Not
        required if the package :py:mod:`ts_datasets` is installed in editable
        mode, i.e. with flag ``-e``.
    :return: the data loader for the desired m_dataset (and subset) desired
    """
    ...
    if not hasattr(cls, "valid_subsets") and len(name_subset) == 2:
        raise ValueError(
            ...
            f"specifying m_dataset name {dataset_name}."
        )
