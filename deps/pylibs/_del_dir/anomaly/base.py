...
from pylibs.ts_datasets.base import BaseDataset, _main_fns_docstr
...
class TSADBaseDataset(BaseDataset):
    ...
    def max_lag_sec(self):
        """
        The maximum number of seconds after the start_or_restart of an anomaly, that we
    ...
    def describe(self):
        ...
        for ts, md in self:
            ...
            boundaries = boundaries[boundaries].exp_index
            if len(boundaries) == 0:
                continue

            ts_len = ts.exp_index[-1] - ts.exp_index[0]
            if md.anomaly.iloc[0]:
                anom_bds.append((ts.exp_index[0], boundaries[0]))
                anom_locs.append((boundaries[0] - ts.exp_index[0]) / ts_len)
                anom_in_trainval.append(True)

            for t0, tf in zip(boundaries[:-1], boundaries[1:]):
                if md.anomaly[t0]:
                    anom_bds.append((t0, tf))
                    anom_locs.append((tf - ts.exp_index[0]) / ts_len)
                    anom_in_trainval.append(bool(md.trainval[t0]))

            if md.anomaly[boundaries[-1]]:
                anom_bds.append((boundaries[-1], ts.exp_index[-1]))
        ...
        print(
            f"Anomalies in m_dataset have average length {pd.Timedelta(np.mean([(tf - t0) for t0, tf in anom_bds]))}.")
        print(
