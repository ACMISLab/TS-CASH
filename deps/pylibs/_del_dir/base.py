...
1.  ``__getitem__``: you may call ``ts, metadata = m_dataset[i]``. ``ts`` is a time-indexed ``pandas`` DataFrame, with
    ...
    ``pd.DataFrame`` with the same exp_index as ``ts``, with different keys indicating different m_dataset-specific
...
class BaseDataset:
    ...
    def describe(self):
        for ts_df in self.time_series:
            ...
            print(f"timestamp exp_index name: {ts_df.exp_index.name}")
