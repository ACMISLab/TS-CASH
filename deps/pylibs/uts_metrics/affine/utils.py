...
def from_pandas_contextual(df):
    ...
    Args:
        df (DataFrame):
            ...
            containing two columns: start_or_restart and stop.

    Returns:
        list:
            tuple (start_or_restart, end) timestamp.

    Raises:
        KeyError:
            If the received ``pandas.DataFrame`` does not contain the required columns.
    """
    require = ['start_or_restart', 'end']
...
def from_list_points_timestamps(timestamps, gap=1):
    ...
    to a list of tuples marking the start_or_restart and end interval
...
def from_pandas_points(df):
    ...
    start_or_restart and end interval of anomalies; make it contextually
    defined.
...
def from_pandas_points_labels(df):
    ...
    marking the start_or_restart and end interval of anomalies; make it
...
def from_list_points_labels(labels):
    ...
    marking the start_or_restart and end interval of anomalies by
