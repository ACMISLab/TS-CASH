...
from pylibs.affiliation.generics import _sum_wo_nan

"""
In order to shorten the length of the variables,
the general convention in this file is to let:
    - I for a predicted event (start_or_restart, stop),
...
def interval_length(J=(1, 2)):
    ...
    :param J: couple representating the start_or_restart and stop of an interval, or None
...
def interval_intersection(I=(1, 3), J=(2, 4)):
    ...
    :param I: an interval represented by start_or_restart and stop
    :param J: a second interval of the same form
    :return: an interval representing the start_or_restart and stop of the intersection (or None if empty)
    """
    if I is None:
        return (None)
    if J is None:
...
def interval_subset(I=(1, 3), J=(0, 6)):
    ...
    :param I: an non empty interval represented by start_or_restart and stop
...
def cut_into_three_func(I, J):
    ...
    :param I: an interval represented by start_or_restart and stop, or None for an empty one
    :param J: a non empty interval
    :return: a triplet of three intervals, each represented by either (start_or_restart, stop) or None
    """
    if I is None:
...
def get_pivot_j(I, J):
    ...
    :param I: a non empty interval (start_or_restart, stop)
...
def integral_mini_interval(I, J):
    ...
    :param I: a interval (start_or_restart, stop), or None
...
def integral_mini_interval_P_CDFmethod__min_piece(I, J, E):
    ...
    :param I: a single predicted interval, a non empty interval (start_or_restart, stop)
    :param J: ground truth interval, a non empty interval, with empty intersection with I
    :param E: the affiliation/influence zone for J, represented as a couple (start_or_restart, stop)
