#!/usr/bin/env python3
# -*- coding: utf-8 -*-
from pylibs.affiliation._affiliation_zone import get_all_E_gt_func, affiliation_partition
from pylibs.affiliation._single_ground_truth_event import affiliation_precision_distance, \
    affiliation_recall_distance, affiliation_precision_proba, affiliation_recall_proba
from pylibs.affiliation.generics import infer_Trange, has_point_anomalies, _sum_wo_nan, _len_wo_nan, \
    read_all_as_events
...
def test_events(events):
    ...
    :param events: list of events, each represented by a couple (start_or_restart, stop)
    ...
    if not all([len(x) == 2 for x in events]):
        raise ValueError('Input `events` should be a list of couples (start_or_restart, stop)')
    if not all([x[0] <= x[1] for x in events]):
        raise ValueError(
            'Input `events` should be a list of couples (start_or_restart, stop) with start_or_restart <= stop')
...
def pr_from_events(events_pred, events_gt, Trange):
    ...
    indicating the start_or_restart and the stop of the event
    ...
    represented as a couple (start_or_restart, stop)
