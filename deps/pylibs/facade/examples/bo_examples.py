...
from exps.e_cash_libs import BayesianFacade
...
n_generations = 3
ef = BayesianFacade(scenario=scenario)
_total_exp=140
_n_init=int(_total_exp*0.25)
for conf in scenario.configspace.sample_configuration(_n_init):
    acc = black_function_1(conf['x'])
    ef.tell(conf, fun_cost=acc, time=1)

for i in tqdm(range(_total_exp-_n_init)):
    conf = ef.ask()
    acc = black_function_1(conf['x'])
    print(f"{conf}={acc}")
    ef.tell(conf, fun_cost=acc, time=1)
