...
class SobolSampling(Sampling):
    ...
    def _sample(self, n_points, n_dim):
        ...
        _X = np.zeros((n_sequence, n_dim), dtype=int)
...
class Sobol(DS):
    def transform(self, X_train, y_train, X_test, y_test, econf: ExpConf):
        ...
        ss = SobolSampling(seed=econf.random_state)
        train_sample_index=(ss.sample(n_points=k, n_dim=1) * X_train.shape[0]).astype('int').ravel()
        # train_sample_index =  ().astype("int").ravel()
